...
PROJ    = toggle
PROJDIR = d:\git\microcontroller-test-code\lc87\toggle
...
OPT_CC  = -CLC87K -OaBcdefhiLrsUv -g -A1 -w555 -s -Ms -I"$(PRODDIR)\include"
OPT_AS  = -Ms -gAHLs
OPT_LK  = -lcs -lfps -lrts -L"$(PRODDIR)\lib" -M -mcfklMoQRSU -OLt -w127 --lsl-file=lc87k.lsl

all : "$(PROJ).hex"

"main.src" : "main.c" "$(PRODDIR)\include\c87.h" "$(PRODDIR)\include\Lc87f1m.sfr" <1502404736>
	@echo Compiling "main.c"
	@"$(PRODDIR)\bin\c87.exe" "main.c" -f<<EOF
	$(XX) $(OPT_CC)
	EOF

"main.obj" : "main.src" <1502400995>
	@echo Assembling "main.src"
	@"$(PRODDIR)\bin\as87.exe" "main.src" -f<<EOF
	$(XX) $(OPT_AS)
	EOF

"$(PROJ).abs" : "main.obj" <1502400995>
	@echo Linking to $@
	@"$(PRODDIR)\bin\l87.exe" -o $@:ieee $(match .obj $!) $(match .a $!) $(OPT_LK)

"$(PROJ).x87" : "$(PROJ).abs" <1502400995>
	@echo Converting to $@
	@"$(PRODDIR)\bin\x87s.exe" $! 

"$(PROJ).hex" : "$(PROJ).x87" <1502400995>
	@echo Converting to $@
