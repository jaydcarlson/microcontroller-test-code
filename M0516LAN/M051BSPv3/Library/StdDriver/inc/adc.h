...
/**
  ...
  * @note On M05xxBN, Burst mode and ADC compare function can not support 2's complement output format, u32Format should be set to 0.
  */
...
/**
  ...
  * @note On M05xxBN, when the ADST bit is cleared to 0, the ADST bit must be kept at 0 at least one ADC peripheral clock period
  *       before setting it to 1 again, otherwise the A/D converter may not work.
  *       If ADST bit is cleared to 0 when ADC is in converting, the BUSY bit will be cleared to 0 immediately, ADC will finish
  *       the current conversion and save the result to the ADDRx register of the enabled channel.
  *       On M05xxDN and M05xxDE, if ADST bit is cleared to 0 when ADC is in converting, the BUSY bit will be cleared to 0 immediately,
